...
<!-- Stylesheets
	...
	<noscript><img height="1" width="1" style="display:none"
	  src="https://www.facebook.com/tr?id=405411949945205&ev=PageView&noscript=1"
	/></noscript>
...
<!--{{ (isset($isWork)!=null?"data-loader-timeout='16000'":"") }}"   data-speed-in=1500 data-animation-out=fadeOut data-speed-out=800 data-loader=1 class="stretched dark side-push-panel side-panel-right sticky-responsive-menu device-xxs"-->
@php
if($locale=='ar')
{
@endphp
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-WVS3H4C"
			...
			<div class="widget clearfix">
				...
				<!-- Primary Navigation
					...
					<nav  class="nav-tree nobottommargin" >
						...
						<ul>
							<li {!! (isset($isAbout)?' class="current"':'') !!}><a href="{{$url}}/about"><div>{{ __('transcript.menu_1') }}</div></a>
</li>
...
<li {!! (isset($isContact)?' class="current"':'') !!}><a href="{{$url}}/contact"><div>{{ __('transcript.menu_7') }}</div></a></li>
					   ...
					   <div style='font-size:22px; color: #BBB; margin-top:25px' >@php
						$cnt = 1;
						$oldLocale =  app()->getLocale();
						...
						if(substr($url,0,2)=='ar' || substr($url,0,2)=='fr' || substr($url,0,2)=='en')
						{
							$url = ""; //substr($url,2, 100);
						}
						if(isset($isWork)  || isset($isBlog))
						{
							//var_dump($oldLocale);
					...
					@foreach($newArray as $lang)
						...
						<a href="/locale/{{strtolower($lang)}}" style='color: #BBB;'>{!! $wordLang !!}</a>
						@endif
						@php
							$cnt++
						@endphp
	...
	@php
		$url = Request::path();
		$removeCookie = 1;
		if($url!='privacypolicy' && $removeCookie===1)
		{
			$removeCookie = 0;
			if(!Session::has('isCount'))
			{
				Session::put('isCount',0);
			}
			if(!Session::has('page'))
			{
			...
			{
				//echo 'notpasok';
				if(Session::get('page') != $url || (Session::get('isCount')>0 && Session::get('page') == $url) )
				{
					if(Session::get('page') != $url)
					{
						$cnt = Session::get('isCount');
						Session::put('isCount', $cnt++);
					}
					$removeCookie = 1;
				}
			}
	...
	{
	@endphp
	<div class='container-fluid nobottommargin nobottompadding overlaycookie' style='background-color:#111'>
		
			<div class='row'>
				<div class='col  nobottommargin nobottompadding' style='position:relative'>
					<i class="icon-line-cross closecookie" style='position:absolute; font-size:16px; right:20px; top:0px; cursor:pointer'></i>
					<p style='font-size:14px; margin-bottom:5px; padding-top:5px' class='container'>We use cookies to provide you with the best experience on our website, <a href='{{ env('APP_URL') }}/privacypolicy' style='color:#fff; text-decoration: underline !important'>find out more here</a>. By continuing to use our website or clicking <a href='/agreecookie' style='color:#fff; text-decoration:underline !important'>Continue</a>, you agree to our use of cookies.</p>
